...
export default function CategoryPage() {
  ...
  const [page, setPage] = useState(parseInt(pageParam || '0'));
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (catRes.ok) {
        ...
        const artRes = await fetch(`${API_URL}/api/public/categories/${slug}/articles?page=${page}&size=12`);
        if (artRes.ok) {
          const artData = await artRes.json();
          const pageData: PageResponse<Article> = artData.data;
    ...
    return (
      <div className="min-h-screen" style={{ backgroundColor: 'var(--bg-light)' }}>
        ...
        <div className="flex flex-col items-center justify-center py-32">
          <h1 className="text-xl mb-4" style={{ color: 'var(--text-dark)' }}>Không tìm thấy danh mục</h1>
          <Link
            href="/articles"
          ...
          <div className="flex items-center gap-6 md:gap-8 h-12 min-w-max">
            ...
            {articleCategories.map((cat) => (
              <Link
                ...
                className={`text-[11px] tracking-[0.05em] uppercase whitespace-nowrap transition-colors py-3 border-b-2 -mb-[1px]`}
                style={{
                  color: cat.slug === slug ? 'var(--text-dark)' : 'color-mix(in srgb, var(--text-dark) 60%, transparent)',
                  borderColor: cat.slug === slug ? 'var(--text-dark)' : 'transparent',
